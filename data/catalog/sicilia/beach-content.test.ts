...
describe("Sicilian master content", () => {
  it("contains one validated content record for every candidate", () => {
    ...
    expect(contentResult.records).toHaveLength(50);
    expect(new Set(contentResult.records.map((record) => record.slug)).size).toBe(50);
  ...
  it("does not make draft candidates eligible for publication", () => {
    ...
    expect(result.blocked).toHaveLength(50);
