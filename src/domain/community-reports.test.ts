...
describe("community report choices", () => {
  it("offers guided details for every report category", () => {
    ...
    expect(getCommunityReportDetailOptions("water").map((option) => option.label)).toContain("Mare calmo");
    expect(getCommunityReportDetailOptions("water").map((option) => option.label)).toContain("Mare mosso");
